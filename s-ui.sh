...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/vrzdrb/s-ui-custom/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/vrzdrb/s-ui-custom/master/install.sh"
...
update_shell() {
    wget -O /usr/bin/s-ui -N --no-check-certificate https://github.com/vrzdrb/s-ui-custom/raw/main/s-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
